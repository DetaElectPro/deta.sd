...
- Project id `feufokxcmlqbgyffqjqr` (`supabase/config.toml`). SQL in `supabase/migrations/` + root `supabase_complete_migration.sql`; edge function in `supabase/functions/send-order-confirmation/`.
...
- **`vite.config.ts` `base: '/deta.sd/'` is load-bearing** for GitHub Pages asset paths (custom domain via `CNAME` = `deta.sd`). Don't change without verifying deploy. `lovable-tagger` runs in development mode only.
